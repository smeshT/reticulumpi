...
# --- 1.5i: reticulumpi-wlan.service (wlan1 AP) + portal patch ------
# hostapd runs on wlan1 (USB dongle). We install reticulumpi-wlan.service
# which brings up wlan1 with static IP 192.168.4.1, replacing the
# upstream reticulumhf-wlan.service (which uses wlan0). We also patch
# the ReticulumHF portal's update_hostapd_config() to write wlan1
# so the portal doesn't overwrite the correct config on wifi saves.
phase "Phase 1.5i: reticulumpi-wlan.service (wlan1 AP) + portal patch"
