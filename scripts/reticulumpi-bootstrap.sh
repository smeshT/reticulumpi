...
# Client-wifi (STA) settings for the Pi's built-in wifi (wlan0).
# The USB wifi dongle (wlan1) runs hostapd as the AP (see Phase 1.5c).
# The built-in wlan0 joins the operator's local wifi so the box has
# internet for ZeroTier, apt, etc. out of the gate.
#
...
# Optional wlan1 USB wifi dongle runs hostapd as AP. If no wlan1 is present
# (e.g. reticulumpi built-in wifi only), this phase is skipped entirely.
# Built-in wlan0 runs STA (Phase 1.5h) and handles all upstream connectivity.
phase "Phase 1.5c: Configure hostapd (SSID='$RETICULUMPI_SSID', country='$RETICULUMPI_WIFI_COUNTRY')"

if [ ! -e /sys/class/net/wlan1 ]; then
    # No wlan1 — stop any stray hostapd running on wlan0 (wrong interface).
    # The built-in wlan0 is STA only; hostapd has no business running here.
    sudo systemctl stop hostapd 2>/dev/null || true
    sudo systemctl disable hostapd 2>/dev/null || true
    ok "no wlan1 (no USB dongle) — hostapd stopped, wlan0 STA mode only"
else
...
# AP runs on wlan1 (USB wifi dongle). Built-in wlan0 runs STA.
...
# --- 1.5g: DEPRECATED (superseded by Phase 1.5h) ----------------------
# This phase previously wrote /etc/wpa_supplicant/wpa_supplicant.conf but
# did NOT enable any wpa_supplicant service, so it had zero effect.
# The actual wlan0 STA configuration is now in Phase 1.5h below.

# --- 1.5h: wlan0 STA — wpa_supplicant + NetworkManager -------------
# The Pi's built-in wifi (wlan0) runs in STA mode: it joins the
# operator's home wifi to get internet. NetworkManager manages wlan0,
# but we must tell NM to leave it alone (unmanaged) so that
# wpa_supplicant@wlan0.service owns it instead. hostapd runs on wlan1.
phase "Phase 1.5h: Configure wlan0 as upstream STA (built-in wifi)"

# Prevent dhcpcd from managing wlan0 (wpa_supplicant handles it instead).
...
EOF
    ...
    # No SSID yet — write a minimal config; operator must configure via
    # the node-portal /connect page after first boot.
    sudo tee /etc/wpa_supplicant/wpa_supplicant-wlan0.conf > /dev/null <<EOF
# ReticulumPi wlan0 STA — generated by reticulumpi-bootstrap.sh
# No client SSID configured; configure via node-portal /connect page.
country=$RETICULUMPI_WIFI_COUNTRY
ctrl_interface=DIR=/var/run/wpa_supplicant GROUP=netdev
update_config=1
EOF
...
# Enable + start wpa_supplicant@wlan0.service so systemd owns wlan0 STA.
sudo systemctl enable wpa_supplicant@wlan0.service 2>/dev/null || true
ok "wpa_supplicant@wlan0.service enabled (wlan0 STA)"

# Mark wlan0 as unmanaged so NetworkManager doesn't compete for it.
# Without this, NM would either refuse to start hostapd on wlan0
# (interface in use) or clobber the wpa_supplicant connection.
...
# --- 1.5i: reticulumpi-wlan.service (optional wlan1 AP) + portal patch ---
# Optional wlan1 USB wifi dongle: hostapd AP on wlan1. If no wlan1, this
# phase is skipped. Built-in wlan0 is STA only (Phase 1.5h) when no dongle.
phase "Phase 1.5i: reticulumpi-wlan.service (optional wlan1 AP) + portal patch"

if [ ! -e /sys/class/net/wlan1 ]; then
    ok "no wlan1 — skipping reticulumpi-wlan.service and portal patch"
else
    # Disable the upstream reticulumhf-wlan.service (uses wlan0).
    sudo systemctl disable reticulumhf-wlan 2>/dev/null || true

    # Install reticulumpi-wlan.service: brings up wlan1 AP on 192.168.4.1.
    sudo tee /etc/systemd/system/reticulumpi-wlan.service > /dev/null <<'EOF'
# ReticulumPi AP interface — wlan1 (USB wifi dongle)
...
EOF
    ...
    # Patch the ReticulumHF portal's update_hostapd_config() to use wlan1.
    # Without this, the portal writes interface=wlan0 when the operator saves
    # wifi settings, overwriting our correct wlan1 config.
    ...
    if [ -f "$WIZARD_APP" ]; then
        if grep -q 'interface=wlan0' "$WIZARD_APP" 2>/dev/null; then
            ...
            ok "portal: already wlan1 or pattern not found"
        fi
    else
        warn "wizard app.py not at $WIZARD_APP — skipping portal patch"
    fi
    ok "wifi roles corrected: wlan0=STA (built-in), wlan1=AP (dongle)"
fi
