...
# --- 1.5g: client wpa_supplicant (DEPRECATED — superseded by Phase 1.5h) ---
# This phase is kept for reference only. The actual wlan0 STA config
# is now in Phase 1.5h which properly wires wpa_supplicant to wlan0
# via systemd (wpa_supplicant@wlan0.service). This phase previously
# wrote /etc/wpa_supplicant/wpa_supplicant.conf but did not enable
# the service, so it had no effect.
...
# --- 1.5h: onboard wifi (wlan0) as upstream STA client -----------
# The Pi's built-in wifi (wlan0) runs in STA mode — it joins the
# operator's home wifi to get internet. The USB dongle (wlan1) runs
# hostapd as the AP. This is the inverse of the ReticulumHF default.
#
# Steps:
#   1. Assign static IP to wlan0 so dhcpcd doesn't clobber it with DHCP
#   2. Write wpa_supplicant.conf wired to wlan0 via systemd
#   3. Tell NetworkManager to leave wlan0 alone (it would otherwise
#      compete with wpa_supplicant and steal the interface)
phase "Phase 1.5h: Configure wlan0 as upstream STA (built-in wifi)"

# Patch /etc/dhcpcd.conf: the ReticulumHF base writes the AP static IP
# block here. We keep it but add denyinterfaces wlan0 so dhcpcd does
# NOT manage wlan0 (wpa_supplicant+systemd handles it instead).
if ! grep -q "^denyinterfaces wlan0" /etc/dhcpcd.conf 2>/dev/null; then
    sudo sed -i '/^interface wlan0/a \
denyinterfaces wlan0' /etc/dhcpcd.conf
    ok "dhcpcd.conf: added denyinterfaces wlan0 (prevents dhcpcd from touching wlan0)"
else
    ok "dhcpcd.conf: denyinterfaces wlan0 already present"
fi

# Wire wpa_supplicant to wlan0 via systemd. On Bookworm/Trixie the
# default is wpa_supplicant@.service which takes the iface name as the
# instance. We create /etc/wpa_supplicant/wpa_supplicant-wlan0.conf
# and enable wpa_supplicant@wlan0.service so systemd starts it.
if [ -n "$RETICULUMPI_CLIENT_SSID" ]; then
    sudo mkdir -p /etc/wpa_supplicant
    sudo tee /etc/wpa_supplicant/wpa_supplicant-wlan0.conf > /dev/null <<EOF
# ReticulumPi client-wifi — onboard wlan0 STA mode
# Generated by reticulumpi-bootstrap.sh
country=$RETICULUMPI_WIFI_COUNTRY
ctrl_interface=DIR=/var/run/wpa_supplicant GROUP=netdev
update_config=1
...
EOF
    ...
    sudo systemctl enable wpa_supplicant@wlan0.service 2>/dev/null || true
    ok "wpa_supplicant@wlan0.service enabled (SSID=$RETICULUMPI_CLIENT_SSID)"
else
    # No client SSID — create a minimal conf so the service at least starts
    sudo mkdir -p /etc/wpa_supplicant
    sudo tee /etc/wpa_supplicant/wpa_supplicant-wlan0.conf > /dev/null <<EOF
# ReticulumPi client-wifi — onboard wlan0 STA mode
# No client SSID configured; wpa_supplicant waits for nmcli or a scan result.
country=$RETICULUMPI_WIFI_COUNTRY
ctrl_interface=DIR=/var/run/wpa_supplicant GROUP=netdev
update_config=1
EOF
    sudo chmod 600 /etc/wpa_supplicant/wpa_supplicant-wlan0.conf
    sudo systemctl enable wpa_supplicant@wlan0.service 2>/dev/null || true
    ok "wpa_supplicant@wlan0.service enabled (no SSID; operator must configure via node-portal)"
fi

# Tell NetworkManager to ignore wlan0. Without this, NM grabs wlan0
# (it manages all interfaces by default) and either:
#   - refuses to start hostapd on wlan0 (interface in use)
#   - or clobbers wpa_supplicant's connection
# We put wlan0 in NM's denied list. This is safe because wlan0
# is dedicated to wpa_supplicant for STA; hostapd runs on wlan1.
...
# ReticulumPi: wlan0 is wpa_supplicant STA, not managed by NetworkManager
[keyfile]
unmanaged-devices=interface-name:wlan0
EOF
    ok "NetworkManager: wlan0 marked unmanaged (wpa_supplicant owns it)"
else
    ok "NetworkManager: wlan0 already unmanaged"
fi

# --- 1.5i: patch ReticulumHF portal + reticulumpi-wlan for wlan1 ----
# The ReticulumHF portal's update_hostapd_config() hardcodes interface=wlan0.
# We patch it to use wlan1 so the portal doesn't overwrite the correct
# config when the operator saves wifi settings. We also create or patch
# reticulumpi-wlan.service to bring up wlan1 as the AP interface.
phase "Phase 1.5i: Patch ReticulumHF portal + reticulumpi-wlan for wlan1"

WIZARD_APP=/opt/reticulumhf/setup-portal/app.py
if [ -f "$WIZARD_APP" ]; then
    # Patch interface=wlan0 -> interface=wlan1 in update_hostapd_config()
    if grep -q 'interface=wlan0' "$WIZARD_APP" 2>/dev/null; then
        sudo sed -i 's/interface=wlan0/interface=wlan1/g' "$WIZARD_APP"
        ok "portal: patched interface=wlan0 -> wlan1 in update_hostapd_config()"
    else
        ok "portal: interface already wlan1 or pattern not found (check manually)"
    fi
    # Also patch reticulumpi-wlan.service references if any
    if grep -q 'wlan0' "$WIZARD_APP" 2>/dev/null; then
        : # no other wlan0 refs expected in portal
    fi
else
    warn "wizard app.py not at $WIZARD_APP — skipping portal patch"
fi

# Install reticulumpi-wlan.service: brings up the AP interface (wlan1)
# with static IP 192.168.4.1, replacing/overriding reticulumhf-wlan
# which uses wlan0. We keep reticulumhf-wlan disabled so it won't
# fight with reticulumpi-wlan over wlan0.
sudo systemctl disable reticulumhf-wlan 2>/dev/null || true
sudo tee /etc/systemd/system/reticulumpi-wlan.service > /dev/null <<'EOF'
# ReticulumPi AP interface service — wlan1 (USB dongle)
# Generated by reticulumpi-bootstrap.sh
[Unit]
Description=ReticulumPi WiFi AP (wlan1)
Documentation=https://github.com/smeshT/reticulumpi
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/bin/sh -c '/sbin/ip link set wlan1 up; /sbin/ip addr add 192.168.4.1/24 dev wlan1 2>/dev/null || true'
ExecStop=/sbin/ip addr del 192.168.4.1/24 dev wlan1

[Install]
WantedBy=multi-user.target
EOF
sudo systemctl daemon-reload
sudo systemctl enable reticulumpi-wlan.service
ok "reticulumpi-wlan.service installed (wlan1, 192.168.4.1)"

ok "wifi roles corrected: wlan0=STA upstream, wlan1=AP dongle"
