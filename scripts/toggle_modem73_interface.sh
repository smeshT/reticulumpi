#!/bin/bash
# toggle_modem73_interface.sh — Enable or disable the Modem73 OFDM modem
# in Reticulum and launch/kill the TUI instance in the VNC Xterminal.
# Single button: flips enabled= + starts/stops the normal TUI modem73
# in an lxterminal on the shared :1 desktop.
# rnsd is restarted after the toggle so it picks up the config change.
...
START_SCRIPT="/home/pi/shared_launcher/scripts/start_modem73_tui.sh"
STOP_SCRIPT="/home/pi/shared_launcher/scripts/stop_modem73_tui.sh"
...
if [ "$target" = "true" ]; then
    ...
    sleep 1
    "$START_SCRIPT" || die "start_modem73_tui.sh failed" 2
else
    "$STOP_SCRIPT" || die "stop_modem73_tui.sh failed" 2
fi
